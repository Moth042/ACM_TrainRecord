...
void moth()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        if (b[i] != -1 && b[i] != 0)
        {
            cgMul(max(1, i - b[i] + 1), i, 1, 0);
            cgMul(min(n, i + 1), min(n, i + b[i] - 1), 1, 0);
        }
    }
    for (int i = 1; i <= n; i++) ans[i] = query(i, i, 1);
    for (int i = 1; i <= n; i++)
    {
        if (b[i] != -1)
        {
            int x1 = -1, x2 = -1;
            if (i - b[i] >= 1) x1 = query(i - b[i], i - b[i], 1);
            if (i + b[i] <= n) x2 = query(i + b[i], i + b[i], 1);
            // cout << x1 << ' ' << x2 << '\n';
            // if (x1 == -1 && x2 == -1)
            // {
            //     cout << -1 << '\n';
            ...
            //     return;
            // }
            // if (x1 == 0 && x2 != 0) ans[i + b[i]] = 1;
            // else if (x1 != 0 && x2 == 0) ans[i - b[i]] = 1;
            // else
            // {
            //     cout << -1 << '\n';
            //     return;
            ...
            if (x1 != -1 && x1 != 0) ans[i - b[i]] = 1;
            if (x2 != -1 && x2 != 0) ans[i + b[i]] = 1;
        }
    }
    for (int i = 1; i <= n; i++)
    {
